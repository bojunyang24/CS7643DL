...
def run():
    ...
    for epoch in range(args.epochs):
        ...
        if valid_acc > best_acc:
            best_model_train_acc = epoch_acc
            best_acc = valid_acc
    ...
    if args.debug:
        print(f"Best Model Training accuracy {best_model_train_acc}")
        print(f"Best Model Validation accuracy {best_acc}")
        print("Best Model Accuracy on Test Data: {accuracy:.4f}".format(accuracy=test_acc))
