...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        b2 = self.weights['b2']
        z1 = np.dot(X, W1) + b1
        sigmoid = self.sigmoid(z1)
        z2 = np.dot(sigmoid, W2) + b2
        s = self.softmax(z2)
        ...
        dl_dz2 = s/num_examples
        self.gradients['W2'] = np.dot(sigmoid.T, dl_dz2)
        self.gradients['b2'] = np.sum(dl_dz2, axis=0)
        dz2_dsigmoid = W2
        dsigmoid_dz1 = self.sigmoid_dev(z1)
        # dz1_dw1 = X
        dl_dsigmoid = np.dot(dl_dz2, dz2_dsigmoid.T)
        dl_dz1 = dl_dsigmoid * dsigmoid_dz1
        self.gradients['W1'] = np.dot(X.T,dl_dz1)
        self.gradients['b1'] = np.sum(dl_dz1, axis=0)
        # self.gradients['W1'] = dl_dz2 dz2_dsigmoid dsigmoid_dz1 X
        # self.gradients['W1'] = dl_dsigmoid dsigmoid_dz1 X
        # self.gradients['W1'] = dl_dz1 X
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
