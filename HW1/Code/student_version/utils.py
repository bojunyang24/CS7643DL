...
def load_mnist_trainval():
    ...
    # train_data = np_data[shuffled_idx[:split]].tolist()
    # train_label = np_label[shuffled_idx[:split]].tolist()

    # val_data = np_data[shuffled_idx[split:]].tolist()
    # val_label = np_label[shuffled_idx[split:]].tolist()
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if shuffle:
        ...
        data, label = [list(d) for d in zip(*labeled_data)]
    batched_data = [np.asarray(data[x:x+batch_size]) for x in range(0, len(data), batch_size)]
    batched_label = [np.asarray(label[x:x+batch_size]) for x in range(0, len(label), batch_size)]
    
    # if shuffle:
    #     labeled_data = list(zip(data, label))
    #     random.shuffle(labeled_data)
    #     data, label = [list(d) for d in zip(*labeled_data)]
    # batched_data = [data[x:x+batch_size] for x in range(0, len(data), batch_size)]
    # batched_label = [label[x:x+batch_size] for x in range(0, len(label), batch_size)]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
