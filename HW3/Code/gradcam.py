...
ggc_grad = GuidedGradCam(model, conv_module)
gcc_attr = compute_attributions(ggc_grad, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/guidedgradcam_captum.png', X, y, class_names, [gcc_attr], ['Guided GradCam'])

# Computing Guided BackProp
gbp_grad = GuidedBackprop(model)
gbp_attr = compute_attributions(gbp_grad, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/guidedbackprop_captum.png', X, y, class_names, [gbp_attr], ['Guided Backprop'])
...
layer_gc_attr_sum = layer_gc_attr.squeeze()
layer_gc_attr_sum = layer_gc_attr.mean(axis=1, keepdim=True).squeeze()
visualize_attr_maps('visualization/layer_gradcam.png', X, y, class_names, [layer_gc_attr_sum], ['Layer GradCam'], attr_preprocess=lambda attr: attr.detach().numpy())
...
layer_conductance_attr_sum = layer_conductance_attr.mean(axis=1, keepdim=True).squeeze()
visualize_attr_maps('visualization/layer_conductance.png', X, y, class_names, [layer_conductance_attr_sum], ['Layer Conductance'], attr_preprocess=lambda attr: attr.detach().numpy())
