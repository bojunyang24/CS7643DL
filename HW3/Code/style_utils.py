...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        optimizer.zero_grad()
        content_loss_ = content_loss(content_weight, feats[content_layer], content_target)
        style_loss_ = style_loss(feats, style_layers, style_targets, style_weights)
        tv_loss_ = tv_loss(img_var, tv_weight)
        loss = content_loss_ + style_loss_ + tv_loss_
        loss.backward()
        if t == decay_lr_at:
