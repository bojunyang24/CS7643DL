...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        loss = scores.gather(1, y_tensor.view(-1,1)).squeeze()
        loss = torch.sum(loss)
        loss.backward()
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        loss = torch.sum(loss)
        loss.backward()
        grad = torch.mean(self.gradient_value.data, (2,3), keepdim=True)
        cam = torch.sum(self.activation_value.data * grad, dim=1)
        cam = cam.numpy()
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
